...
class LoginTests(unittest.TestCase):
    ...
    def _welcomeloginframe(self):
        ...
        loginfirst_page = self.driver.find_element_by_xpath('//UIAApplication[1]/UIAWindow[1]/UIAImage[1]')
        # 验证欢迎登陆的标签
        welcomeloginlabel = self.driver.find_elements_by_name('欢迎登陆')
        # 验证时区按钮和默认的时区标签
        timezone = self.driver.find_element_by_xpath('//UIAApplication[1]/UIAWindow[1]/UIAButton[1]')
        defaulttimezone = self.driver.find_element_by_xpath('//UIAApplication[1]/UIAWindow[1]/UIAStaticText[2]')
        # 验证手机号码输入栏
        ...
        # 验证登录按钮
        loginbut = self.driver.find_element_by_xpath('//UIAApplication[1]/UIAWindow[1]/UIAButton[2]')
        # 验证忘记密码链接按钮
        lostpassword = self.driver.find_element_by_xpath('//UIAApplication[1]/UIAWindow[1]/UIAButton[3]')
        # 验证关闭返回按钮
        closebut = self.driver.find_element_by_xpath('//UIAApplication[1]/UIAWindow[1]/UIAButton[4]')
        # 打印元素验证结果
        logging.info('欢迎登陆页面元素验证，没有问题')
